...
Example:
	safrochaind export-derive-balances ../genesis.json ../snapshot.json
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Produce the map of address to total safrochain balance, both staked and UnbondingStake
			...
			for _, unbonding := range stakingGenesis.UnbondingDelegations {
				...
				unbondingSafrochains := sdkmath.NewInt(0)
				for _, entry := range unbonding.Entries {
					unbondingSafrochains = unbondingSafrochains.Add(entry.Balance)
				}

				acc.UnbondingStake = acc.UnbondingStake.Add(unbondingSafrochains)
			...
			for _, delegation := range stakingGenesis.Delegations {
				...
				stakedSafrochain := delegation.Shares.MulInt(val.Tokens).Quo(val.DelegatorShares).RoundInt()

				acc.Staked = acc.Staked.Add(stakedSafrochain)
			...
			for _, addr := range slices.Sorted(maps.Keys(snapshotAccs)) {
				...
				account.TotalBalances = sdk.NewCoins().
					...
					Add(sdk.NewCoin("usaf", account.Staked)).
					Add(sdk.NewCoin("usaf", account.UnbondingStake)).
					Add(account.Bonded...)
...
Example:
	safrochaind staked-to-csv ../balances.json ../airdrop.csv
`,
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx := client.GetClientContextFromCmd(cmd)
