...
// v27Chain is the first version of safrochain that will be upgraded from
var v27Chain = ibc.DockerImage{
	Repository: SafrochainMainRepo,
	Version:    "v27.0.0",
	UIDGID:     "1025:1025",
}

// v28Chain is the second version of safrochain that will be upgraded from
...
func SubmitBuilderParamsUpdate(t *testing.T, ctx context.Context, chain *cosmos.CosmosChain, user ibc.Wallet) string {
	govModule := sdk.MustAccAddressFromBech32("safrochain10d07y265gmmuvt4z0w9aw880jnsr700jvss730")
	...
	updateParamsMsg := []cosmos.ProtoMessage{
		&buildertypes.MsgUpdateParams{
			...
			Params: buildertypes.Params{
				...
				ReserveFee:             sdk.NewCoin("usaf", sdkmath.NewInt(1)),
				MinBidIncrement:        sdk.NewCoin("usaf", sdkmath.NewInt(1000)),
				MaxBundleSize:          100,
	...
	proposal, err := chain.BuildProposal(
		...
		sdk.MustBech32ifyAddressBytes("addr_safro", user.Address()),
		false)
