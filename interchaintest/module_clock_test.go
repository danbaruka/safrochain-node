...
// TestSafrochainClock ensures the clock module auto executes allowed contracts.
func TestSafrochainClock(t *testing.T) {
	t.Parallel()
	...
	// Setup contract
	_, contractAddr := helpers.SetupContract(t, ctx, safrochain, user.KeyName(), "contracts/clock_example.wasm", `{}`)

	// Ensure config is 0
	res := helpers.GetClockContractValue(t, ctx, safrochain, contractAddr)
	fmt.Printf("- res: %v\n", res.Data.Val)
	require.Equal(t, uint32(0), res.Data.Val)

	// Register the contract
	_, err := helpers.RegisterClockContract(t, ctx, safrochain, user, contractAddr)
	require.NoError(t, err)

	// Validate contract is not jailed
	contract := helpers.GetClockContract(t, ctx, safrochain, contractAddr)
	...
	_, err = helpers.UnregisterClockContract(t, ctx, safrochain, user, contractAddr)
	require.NoError(t, err)
	helpers.ValidateNoClockContract(t, ctx, safrochain, contractAddr)
	...
	// Setup contract
	_, contractAddr = helpers.SetupContract(t, ctx, safrochain, user.KeyName(), "contracts/clock_example_high_gas.wasm", `{}`, "--admin", user.FormattedAddress())
	...
	helpers.MigrateContract(t, ctx, safrochain, user.KeyName(), contractAddr, "contracts/clock_example_migrate.wasm", `{}`)

	// Unjail the contract
	_, err = helpers.UnjailClockContract(t, ctx, safrochain, user, contractAddr)
	require.NoError(t, err)
	...
	// Setup contract
	_, contractAddr = helpers.SetupContract(t, ctx, safrochain, user.KeyName(), "contracts/clock_example_no_sudo.wasm", `{}`)
...
func SubmitParamChangeProp(t *testing.T, ctx context.Context, chain *cosmos.CosmosChain, user ibc.Wallet, gasLimit uint64) string {
	govAcc := "safrochain10d07y265gmmuvt4z0w9aw880jnsr700jvss730"
