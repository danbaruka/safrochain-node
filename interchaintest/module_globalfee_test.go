...
// TestSafrochainGlobalFee
func TestSafrochainGlobalFee(t *testing.T) {
	t.Parallel()

	cfg := safrochainConfig
	cfg.GasPrices = "0.003usaf" // this is used in the faucet cmd, must match initial globalfee
	...
	users := interchaintest.GetAndFundTestUsers(t, ctx, "default", initFunds, safrochain, safrochain)
	sender := users[0]
	receiver := users[1].FormattedAddress()

	// fail: send 1 token to the receiver, no fee provided.
	std := bankSendWithFees(t, ctx, safrochain, sender, receiver, "1"+nativeDenom, "0"+nativeDenom, 200000)
	require.Contains(t, std, "no fees were specified")

	// fail: not enough fees
	std = bankSendWithFees(t, ctx, safrochain, sender, receiver, "1"+nativeDenom, "1"+nativeDenom, 200000)
	require.Contains(t, std, "insufficient fees")

	// fail: wrong fee token
	std = bankSendWithFees(t, ctx, safrochain, sender, receiver, "1"+nativeDenom, "1NOTATOKEN", 200000)
	require.Contains(t, std, "fee denom is not accepted")

	// success: send with enough fee (200k gas * 0.003 = 600)
	std = bankSendWithFees(t, ctx, safrochain, sender, receiver, "2"+nativeDenom, "600"+nativeDenom, 200000)
	require.Regexp(t, regexp.MustCompile(`raw_log:\s*""`), std)
	require.Regexp(t, regexp.MustCompile(`code:\s*0`), std)

	afterBal, err := safrochain.GetBalance(ctx, receiver, nativeDenom)
	require.NoError(t, err)
	require.Equal(t, initFunds.Add(sdkmath.NewInt(2)), afterBal)

	// param change proposal (lower fee), then validate it still works
	propID := submitGlobalFeeParamChangeProposal(t, ctx, safrochain, sender)
	propIDUint64, err := strconv.ParseUint(propID, 10, 64)
	require.NoError(t, err, "error converting propID to int64")
	helpers.ValidatorVote(t, ctx, safrochain, propIDUint64, 25)

	// success: validate the new value is in effect (200k gas * 0.005 = 200usaf)
	std = bankSendWithFees(t, ctx, safrochain, sender, receiver, "3"+nativeDenom, "1000"+nativeDenom, 200000)
	require.Regexp(t, regexp.MustCompile(`raw_log:\s*""`), std)
	require.Regexp(t, regexp.MustCompile(`code:\s*0`), std)
...
func bankSendWithFees(t *testing.T, ctx context.Context, chain *cosmos.CosmosChain, from ibc.Wallet, toAddr, coins, feeCoin string, gasAmt int64) string {
	cmd := []string{
		"safrochaind", "tx", "bank", "send", from.KeyName(), toAddr, coins,
		"--home", chain.HomeDir(),
...
func submitGlobalFeeParamChangeProposal(t *testing.T, ctx context.Context, chain *cosmos.CosmosChain, user ibc.Wallet) string {
	upgradeMsg := []cosmos.ProtoMessage{
		&globalfeetypes.MsgUpdateParams{
			Authority: "safrochain10d07y265gmmuvt4z0w9aw880jnsr700jvss730",
